...
import { InternInscriptionModel, StudentModel, UserModel } from "@/models";
...
export const StudentsIncriptions = () => {
  ...
  const getData = async () => {
    try {
      const { data: inscr } = await enagApi.get<InternInscriptionModel[]>(
  ...
  const saveData = async () => {
    try {
      ...
      selectedRows.map((row) => {
        const inscriptionTemp: InternInscriptionModel = {
          id: 0,
          course_id: Number(id),
          student_id: row.student.id,
          date: new Date(),
        };
        inscriptionPromises.push(newInternInscription(inscriptionTemp));
      });
